...
namespace S3Explorer.com.amazon.s3
{
    ...
    public class AWSAuthConnection
    {
        ...
        public delegate void UploadProgressChangedEventDelegate(decimal percentComplete, bool done);

        public event UploadProgressChangedEventDelegate UploadProgressChanged;
        ...
        public Response put(string bucket, string key, S3Object obj, SortedList headers )
        {
            ...
            request.Timeout = 720 * 60 * 1000;

            Stream stream = request.GetRequestStream();
            stream.WriteTimeout = 720 * 60 * 1000;

            byte[] data;

            for (int i = 0; i < obj.Bytes.Length; i+= 10240)
            {
                ChangeDownloadProgress(((decimal)i) / obj.Bytes.Length, false);

                data = obj.Bytes.Skip(i).Take(10240).ToArray();
                stream.Write(data, 0, data.Length);
            }

            ChangeDownloadProgress(1, true);
        ...
        private void ChangeDownloadProgress(decimal progress, bool done)
        {
            if (UploadProgressChanged != null)
            {
                UploadProgressChanged.Invoke(progress, done);
            }
        }
